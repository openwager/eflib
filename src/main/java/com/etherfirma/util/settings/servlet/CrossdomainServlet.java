...
public class CrossdomainServlet extends HttpServlet
{
	...
	protected static Document createDocument (final JsonObject s) throws ServletException, IOException
	{
		try {
			...
			DocumentType doctype = domImpl.createDocumentType ("cross-domain-xml", "SYSTEM", "http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd");
			...
			for (int i = 0; i < ahrh.size (); i++) {
				...
				final String headers = o.getString ("headers");
				if (headers == null) {
					throw new IOException ("Missing field: headers");
				}
	...
	public static void dump (final Element el, final OutputStream os) throws Exception
	{
	...
	public static byte [] dump (final Element el) throws Exception
	{
